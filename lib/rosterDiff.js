...
// Doc-ID-safe normalized email — used as the member doc ID for orgs that
// have no MHSP-style patrol ID at all (e.g. Armadillo Mountain), where
// Troopiter email is the only stable identifier a roster row carries.
...
export function computeDiff(csvRows, firestoreMembers) {
  const csvById = Object.fromEntries(csvRows.map(r => [r.id, r]))
 ...
 * Troopiter integration's rehearsal org, has none) — the member doc's `id`
 * is the MHSP Number when present, falling back to the normalized email
 * when it's the only identifier a row carries. Rows with neither are
 * skipped: there's no signal to key a doc on at all.
 */
export function parseCsvRows(rawRows) {
  return rawRows
    .map(row => {
      ...
      const id = mhspNumber || normalizeEmail(email)
      if (!id) return null
