...
// Sent to people picked in the shift-member map picker (issue #225) when a
// driver posts a shift-scoped ride or a rider posts a shift-scoped request.
// `ride` here is deliberately a minimal shape (departure/arrival/date/time)
// shared by both a real ride and a request doc, not the full rideTable() rows
// — a request has no arrival_time/return_departure_time to show.
export async function sendShiftInviteEmail({ to, inviterName, isDriver, ride, shiftTitle, deepLink }) {
  const [departure, arrival] = await Promise.all([
    resolveLocationServer(ride.departure),
    resolveLocationServer(ride.arrival),
  ])
  const roleLine = isDriver
    ? `<strong>${esc(inviterName)}</strong> is driving to <strong>${esc(shiftTitle)}</strong> and invited you to ride along.`
    : `<strong>${esc(inviterName)}</strong> is looking for a ride to <strong>${esc(shiftTitle)}</strong> and invited you to offer one.`
  return sendEmail({
    to,
    subject: `${inviterName} invited you to carpool to ${shiftTitle}`,
    html: layout(`
      <p>Hi,</p>
      <p>${roleLine}</p>
      <table style="border-collapse:collapse;margin:12px 0;font-size:14px">
        <tr><td style="padding:4px 16px 4px 0;font-weight:600;color:#374151;white-space:nowrap">Date</td><td style="padding:4px 0;color:#111827">${esc(ride.departure_date)}</td></tr>
        <tr><td style="padding:4px 16px 4px 0;font-weight:600;color:#374151;white-space:nowrap">Departure</td><td style="padding:4px 0;color:#111827">${esc(departure)}</td></tr>
        <tr><td style="padding:4px 16px 4px 0;font-weight:600;color:#374151;white-space:nowrap">${isDriver ? 'Departs at' : 'Requested pickup'}</td><td style="padding:4px 0;color:#111827">${esc(ride.departure_time)}</td></tr>
        <tr><td style="padding:4px 16px 4px 0;font-weight:600;color:#374151;white-space:nowrap">Arrival</td><td style="padding:4px 0;color:#111827">${esc(arrival)}</td></tr>
      </table>
      <p><a href="${esc(deepLink)}" style="display:inline-block;background:#1d4ed8;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:600">View Dashboard</a></p>
      <p>— MHSP Ride</p>
    `),
  })
}
