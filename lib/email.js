...
export async function sendBookingCanceledByPassengerEmail({ driver, passenger, ride, bookedSeats }) {
  return sendEmail({
    to: driver.email,
    subject: `Booking canceled · ${passenger.fullname} released ${bookedSeats} seat${bookedSeats !== 1 ? 's' : ''}`,
    html: layout(`
      <p>Hi ${driver.fullname},</p>
      <p><strong>${passenger.fullname}</strong> has canceled their booking on your ride, freeing up <strong>${bookedSeats} seat${bookedSeats !== 1 ? 's' : ''}</strong>:</p>
      ${rideTable(ride)}
      <p><strong>Passenger contact:</strong> ${passenger.phone || 'No phone on file'} · ${passenger.email}</p>
      <p><a href="https://mhspride.com/dashboard/rides" style="display:inline-block;background:#1d4ed8;color:#fff;padding:10px 20px;border-radius:6px;text-decoration:none;font-weight:600">View My Offered Rides</a></p>
      <p>— MHSPRide</p>
...
export async function sendBookingCanceledConfirmationEmail({ passenger, ride }) {
  return sendEmail({
    to: passenger.email,
    subject: `Booking canceled · ${resolveLocation(ride.departure)} → ${resolveLocation(ride.arrival)} on ${ride.departure_date}`,
    html: layout(`
      <p>Hi ${passenger.fullname},</p>
