...
- **Dashboard** — the single home screen: today's rides at a glance, scheduled rides (offered + booked), available rides per favorited network, and ride history — each list paginated 10/page with Previous/Next controls. Available Rides has date/origin/destination filters plus a driver/origin/destination search dialog and a "Favorite drivers" toggle (issue #165), applied uniformly across every favorited network's list at once, with one-click clearing
...
- **Achievement badges** (issue #143) — a catalog of milestone, activity, and whimsical badges (`lib/badges/catalog.js`) awarded automatically as members offer/book rides, favorite drivers/riders/networks, upload a photo, switch to dark mode, post or fulfill ride requests, and more. Simple actions award server-side the moment they happen; aggregate/time-based badges are evaluated lazily on dashboard load (throttled to once per 10 minutes). Newly-earned badges surface as a celebration dialog (confetti, one badge at a time with a Next arrow) the moment they're earned, and the full set is visible on the Profile page's Badges card, below the Vehicles section. Members who'd rather not participate can independently check "Hide badges on my profile" and/or "Hide badge notifications" (issue #177) — badges keep being tracked silently in the background either way, so unchecking notifications later replays anything earned in the meantime as the same celebration dialog. These used to be a single combined checkbox; members who had it checked keep notifications hidden after the split (`hide_badge_notifications` is written once, mirroring `hide_badges`, the first time their doc is seen without it). The four favorite-rider badges mirror the existing favorite-driver ones; High-5 Rider has its own custom artwork (issue #167), the other three intentionally reuse their driver-favorite counterpart's art indefinitely
...
**Orgs without MHSP-style patrol ID numbers at all** (e.g. Armadillo Mountain, the Troopiter-integration rehearsal org) aren't a separate code path — `parseCsvRows()` (`lib/rosterDiff.js`) just falls back to the normalized email as the member doc's ID when a row has no `MHSP Number` column value, and the same diffing/import/registration logic runs unchanged on top of that. `verify-membership` mirrors this on the registration side: the "MHSP Member Number" field is optional, and when it's left blank, matching falls back to last name + Troopiter email directly against the email-keyed member doc. A single CSV import can freely mix rows that have MHSP #s with rows that don't.
