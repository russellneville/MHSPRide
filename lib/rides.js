...
// Booking counterpart to computeRideStatus: same date/time-based windowing,
// but keyed off booking_status instead of ride_status since a booking is a
// separate document from the ride it's for.
export function computeBookingStatus(booking, now = new Date()) {
  if (isCanceledStatus(booking?.booking_status)) return "canceled";

  const departure = getBookingDeparture(booking);
  const end = getRideEndTime(booking);
