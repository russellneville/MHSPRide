...
/**
 * Adds `minutes` to a "HH:MM" time string, wrapping past midnight. Shared by
 * estimateArrival below (precomputed predefined-location pairs) and the
 * live Directions-API path in hooks/use-estimated-arrival.js (any pair
 * involving a free-text address, which isn't in the precomputed table).
 */
...
export async function estimateArrival(departureTime, fromId, toId) {
  ...
  return addMinutesToTime(departureTime, minutes)
}
